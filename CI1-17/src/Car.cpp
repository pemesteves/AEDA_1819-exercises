...
bool Car::operator < (const Car &car) const{
	if (brand < car.getBrand())
		return true;
	if (brand == car.getBrand() && price >= car.getPrice())
		return true;
...
FuelCar::FuelCar(string brand, string model, float price,
float tank, float l_100km): Car(brand, model){
	this->tank = tank;
...
ElectricCar::ElectricCar(string brand, string model, float price,
float battery, float kWh_100km): Car(brand, model){
	setPrice(price);
